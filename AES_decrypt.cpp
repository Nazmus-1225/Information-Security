...
unsigned char mcMatrix[4][4]={{0x0e,0x0b,0x0d,0x09},{0x09,0x0e,0x0b,0x0d},{0x0d,0x09,0x0e,0x0b},{0x0b,0x0d,0x09,0x0e}};
unsigned char expanded_key[44][4];
unsigned char roundConstants[]={0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x080,0x1B,0x36};
string key;
unsigned char cipherText[]={0x3B,0x88,0x12,0xAB,0x66,0x84,0x18,0x7D,0xC6,0x6E,0xF7,0x18,0x68,0x59,0xFC,0xAA};
...
void blockToState(unsigned char* cText,int block){
    int i=block*16;
    int j=i+16;
    int k=0,l=0;
...
void stateLeftCircularShift(int row){
    ...
    int i;
    temp=state[row][3];
    for(i=3;i>0;i--){state[row][i]=state[row][i-1];}
    state[row][0]=temp;
...
void Decryption(unsigned char* pText,int block){
    int round=10,i,j,k,l;
...
void print(unsigned char* pText,int n){
     int i,j;
     cout<<"KEYS"<<endl;
    for(i=0;i<44;i++){
